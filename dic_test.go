...
func TestSystemDicType01(t *testing.T) {
	expected := "IPA"
	if sysDicType != expected {
		t.Fatalf("got %v, expected %v\n", sysDicType, expected)
	}
}

func TestSystemDicPath01(t *testing.T) {
	expected := "dic/ipa"
	if sysDicPath != expected {
		t.Errorf("got %v, expected %v\n", sysDicPath, expected)
	}
}

func TestSystemDicMorphs01(t *testing.T) {
	sysDic := NewSysDic()
	expected := 392126
	c := len(sysDic.Morphs)
	if c != expected {
...
func TestSystemDicContents01(t *testing.T) {
	sysDic := NewSysDic()
	expected := 392126
	c := len(sysDic.Contents)
	if c != expected {
		t.Errorf("got %v, expected %v\n", c, expected)
	}
}
...
func TestSystemDicCharClass01(t *testing.T) {
	...
	if !reflect.DeepEqual(sysDic.CharClass, expected) {
		t.Errorf("got %v, expected %v\n", sysDic.CharClass, expected)
	}
}

func TestSystemDicCharCategory01(t *testing.T) {
	const (
		DEFAULT      = 0
		SPACE        = 1
		KANJI        = 2
	...
	for _, cr := range testSet {
		category := sysDic.CharCategory[cr.input]
