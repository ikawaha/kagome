...
func newLattice() (la *lattice) {
	la = new(lattice)
	la.dic = NewSysDic()
	la.pool = newNodePool(_INIT_NODE_BUFFER_SIZE)
	return
}
...
func (la *lattice) build(input string) {
	...
	la.addNode(0, BOSEOS_ID, 0, DUMMY, input[0:0])
	la.addNode(rc+1, BOSEOS_ID, rc, DUMMY, input[rc:rc])
...
func (la *lattice) forward() {
	for i, size := 1, len(la.list); i < size; i++ {
		...
		for index, target := range currentList {
			...
			if len(prevList) == 0 {
				la.list[i][index].cost = _MAX_INT32
				continue
			}
